...
class BaseDataset(Dataset):
    def __init__(self, colored_pc, pc_dims):
    ...
    def augment_training_data(self, image, depth, model=None):
        ...
        if model is not None:
            ...
            pc_gt = model.sample_points_poisson_disk(number_of_points=1024)

        self.horizontal_transform.clear()
        self.vertical_transform.clear()

        if self.colored_pc:
            pc = self.get_pointcloud(depth, image)
        else:
            pc = self.get_pointcloud(depth)

        self.count += 1

        return image, pc, pc_gt
